...
入力された鍵の長さ: ${privateKeyInput.length}文字`)
  ...
  const showSecurityConfirmation = () => {
...
⚠️ 重要な注意事項：
• 実際の資産が入ったアカウントの秘密鍵は絶対に使用しないでください
• テスト専用の空のアカウントの秘密鍵のみ使用してください
• この操作は取り消すことができません

テスト用アカウントの秘密鍵であることを確認した上で「OK」を押してください。`
      : `🚨 Security Confirmation 🚨

Are you sure you want to create NFTs with this private key?

⚠️ CRITICAL WARNINGS:
• NEVER use private keys from accounts with real assets
• Only use private keys from empty test accounts
• This operation cannot be undone

Please confirm this is a TEST ACCOUNT private key before clicking "OK".`
    
    return window.confirm(message)
  }

  const showBulkMintConfirmation = (quantity: number) => {
    const timeEstimate = quantity === 100 ? '約5-10分' : 
                        quantity === 1000 ? '約50分-1時間30分' : 
                        quantity === 10000 ? '約8-15時間' : '数分'
    
    const timeEstimateEn = quantity === 100 ? 'about 5-10 minutes' : 
                          quantity === 1000 ? 'about 50 minutes - 1.5 hours' : 
                          quantity === 10000 ? 'about 8-15 hours' : 'a few minutes'
    
    const message = language === 'ja' 
      ? `⏰ 大量作成の時間確認

${quantity.toLocaleString()}個のNFTを作成します。
予想時間: ${timeEstimate}

この処理は時間がかかりますが、続行しますか？

※途中で停止することも可能です`
      : `⏰ Bulk Creation Time Confirmation

You are about to create ${quantity.toLocaleString()} NFTs.
Estimated time: ${timeEstimateEn}

This process will take a long time. Do you want to continue?
...
※You can stop the process at any time`
  ...
  const handleMintNFTs = async () => {
    if (useEnvKey) {
      ...
      // 大量作成の場合は時間確認
      if (quantity >= 100) {
        if (!showBulkMintConfirmation(quantity)) {
          return
        }
      }
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={language === 'ja' 
                      ? '例: solana-keygen newで生成したBase58秘密鍵を入力'
                      : 'Example: Enter Base58 private key generated by solana-keygen new'
                    }
                  />
                  
                  {/* 強化されたセキュリティ警告 */}
                  <div className="mt-2 p-3 bg-red-100 dark:bg-red-900/30 border-l-4 border-red-500 rounded-r-lg">
                    <div className="flex items-start gap-2">
                      <span className="text-red-500 text-lg font-bold">🚨</span>
                      <div>
                        <p className="text-sm font-bold text-red-800 dark:text-red-300">
                          {language === 'ja' 
                            ? '【重要】実際の資産が入ったアカウントの秘密鍵は絶対に入力禁止！'
                            : '【CRITICAL】NEVER enter private keys from accounts with real assets!'
                          }
                        </p>
